...
/* ---------------- Thread ----------------- */
/* Use FreeRTOS native xTaskCreate (works on most STM32CubeIDE + FreeRTOS setups) */
int ThreadStart(Thread* thread, void (*fn)(void*), void* arg)
{
    BaseType_t rc;
    const uint16_t usTaskStackSize = configMINIMAL_STACK_SIZE * 5; /* adjust if needed */
    UBaseType_t uxTaskPriority = uxTaskPriorityGet(NULL);

    rc = xTaskCreate((TaskFunction_t)fn,
                     "MQTTTask",
                     usTaskStackSize,
                     arg,
                     uxTaskPriority,
                     &thread->task); /* thread->task is of type TaskHandle_t in Paho examples */
...
/* ---------------- Timer ----------------- */
/* Keep FreeRTOS timeout helpers used by many Paho ports */
...
{
    /* xTaskCheckForTimeOut will modify xTicksToWait to remaining ticks */
    xTaskCheckForTimeOut(&timer->xTimeOut, &timer->xTicksToWait);
    if (timer->xTicksToWait < 0) return 0;
    return (int)(timer->xTicksToWait * portTICK_PERIOD_MS);
}

char TimerIsExpired(Timer* timer)
{
    return (xTaskCheckForTimeOut(&timer->xTimeOut, &timer->xTicksToWait) == pdTRUE) ? 1 : 0;
}
